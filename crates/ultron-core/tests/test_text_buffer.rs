...
fn line_length() {
    let raw = "Hello world";
    let buffer = TextBuffer::new_from_str(raw);
...
fn test_cut_text_with_cjk() {
    let raw = "Hello 文件系统 world";
    let mut buffer = TextBuffer::new_from_str(raw);
...
fn ensure_line_exist5() {
    let mut buffer = TextBuffer::new_from_str("");
...
fn test_split_line_at_point() {
    let raw = "Hello world";
    let text_buffer = TextBuffer::new_from_str(raw);
